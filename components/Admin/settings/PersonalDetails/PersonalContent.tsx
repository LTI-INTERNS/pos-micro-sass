...
export default function PersonalContent({ userRole }: PersonalContentProps) {
  ...
  // REQUIREMENT: Disable email field for ADMIN and MANAGER
  const editFields: EditField[] = [
    { name: "name", label: "Name" },
    { 
      name: "email", 
      label: "Email", 
      readOnly: normalizedRole === "ADMIN" || normalizedRole === "MANAGER" 
    },
          ...
          <div className="px-6 py-2">
            <div className="flex justify-left">
              <ActionButton
                ...
                fullWidth={false}
                className="w-[220px]"
                onClick={() => alert("Change Password request sent.")}
              />
            </div>
          </div>
        </section>
...
function SettingsRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="grid grid-cols-12 items-center py-3 border-b border-gray-100">
      <div className="col-span-4 sm:col-span-3 text-sm font-semibold text-gray-900">{label}</div>
      <div className="col-span-8 sm:col-span-9 text-sm font-semibold text-gray-900">{value}</div>
    </div>
  );
}

/**
 * Helper Component for password input rows
 */
