...
import type { AiInsightResponse, SalesPayload } from "@/types/ai-insight.types";
import { TrendingUp, RefreshCcw } from "lucide-react";

type Props = { branchId?: string };

export default function SalesAnalysisContent({ branchId }: Props) {
  const [insight,    setInsight]    = useState<AiInsightResponse<SalesPayload>>(null);
  ...
  const fetchInsight = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await aiInsightService.getSalesInsight(branchId);
      setInsight(data);
    } catch {
  ...
  const handleGenerate = async () => {
    ...
    setError(null);
    try {
      const data = await aiInsightService.generateSalesInsight(branchId);
      setInsight(data);
    } catch {
        ...
        <>
          ...
          <div className="flex flex-col md:flex-col lg:flex-row w-full gap-4">
            <div className="w-full lg:flex-3 min-w-0">
              <SalesForecastChart data={payload.forecastPoints} />
            </div>
          ...
          <div className="w-full lg:flex-3 min-w-0 pt-5">
            <SalesInsightsMetrics items={payload.metrics} />
          </div>
          ...
          <div className="pt-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            >
              ...
              {generating ? "Regenerating..." : "Regenerate Sales Analysis"}
            </button>
          </div>
        </>
      )}
