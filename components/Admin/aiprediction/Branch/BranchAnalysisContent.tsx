...
import type { AiInsightResponse, BranchPayload } from "@/types/ai-insight.types";
import { TrendingUp, RefreshCcw } from "lucide-react";

type Props = { branchId?: string };

export default function BranchAnalysisContent({ branchId }: Props) {
  const [insight,    setInsight]    = useState<AiInsightResponse<BranchPayload>>(null);
  ...
  const fetchInsight = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await aiInsightService.getBranchInsight(branchId);
      setInsight(data);
    } catch {
  ...
  const handleGenerate = async () => {
    ...
    setError(null);
    try {
      const data = await aiInsightService.generateBranchInsight(branchId);
      setInsight(data);
    } catch {
        ...
        <>
          ...
          <div className="flex flex-col md:flex-col lg:flex-row w-full gap-4">
            <div className="w-full lg:flex-3 min-w-0">
              <BranchPerformanceChart data={payload.forecastPoints} />
            </div>
          ...
          <div className="pt-5">
            <div className="w-full bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Branch Forecast Overview</h3>
              <BranchForecastCard branches={payload.branches} />
            </div>
          </div>
          ...
          <div className="pt-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            >
              ...
              {generating ? "Regenerating..." : "Regenerate Branch Analysis"}
            </button>
          </div>
        </>
      )}
